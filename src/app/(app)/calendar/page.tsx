...
export default function CalendarPage() {
  ...
  async function loadMonthData() {
    ...
    // Events — visibility filtered for staff
    ...
    // Leaves — staff sees own, HR/Mgmt sees all
    let leaveQ = supabase.from('leaves').select('*')
    ...
    // Bookings — staff sees own, HR/Mgmt sees all
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {isHrOrMgmt
              ...
              : 'Your schedule — events, leaves, and public holidays'}
          </p>
        </div>
        ...
        <div className="xl:w-80 flex-shrink-0 space-y-4">
          ...
          {selectedDate ? (
            <div className="bg-white border border-gray-100 rounded-2xl p-5">
                  ...
                  <div className="space-y-2">
                    {selLeaves.map(l => (
                      <div key={l.id} className={`p-3 rounded-xl border ${
                        ...
                        {isHrOrMgmt && <p className="text-[10px] text-slate-500 mt-0.5">{l.type} · {l.department}</p>}
              ...
              {profile && (
                <button onClick={() => openAddEvent(selectedDate)}
                  className="w-full mt-2 px-3 py-2 bg-indigo-50 text-indigo-600 text-xs font-semibold rounded-xl hover:bg-indigo-100 transition">
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-5">
              ...
              <p className="text-xs text-slate-400">Click any date on the calendar to see events, leaves, and bookings for that day.</p>
            </div>
          )}
